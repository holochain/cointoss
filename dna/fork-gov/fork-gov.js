...
function genesis()
{
    ...
    // check for existence of the Application Development Collective
    try { get(AppDevCollectiveId) }
    catch(err) 
    { 
        if (err.holochainMessage == "hash not found") // doesn't exist! Initialize it.
            initializeAppDevCollective();
        else
...
UTILITY FUNCTIONS
**************/
function initializeAppDevCollective()
{
    ...
    // add this agent as the first governor
